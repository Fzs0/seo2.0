...
def _executor() -> SocialExecutorClient:
    ...
    return SocialExecutorClient(
        ...
        shared_secret=settings.social_executor_shared_secret,
    )
...
async def prepare_social_job(session: AsyncSession, job_id: str, *, business_id: str) -> dict[str, Any]:
    ...
    if result.get("status") != "awaiting_review" or not result.get("confirmation_token"):
        await _mark_manual(session, row, "executor did not return an awaiting_review confirmation")
        return {"ok": False, "job_id": job_id, "status": "manual_required"}
    ...
    await session.execute(
            text("""
                UPDATE seo_agent.social_publish_jobs
...
async def confirm_social_job(session: AsyncSession, job_id: str, *, business_id: str) -> dict[str, Any]:
    ...
    if result.get("status") != "published" or not _valid_post_url(row["platform"], result.get("post_url")):
        await _mark_manual(session, row, "executor did not verify a real platform post URL")
        return {"ok": False, "job_id": job_id, "status": "manual_required"}
    post_url = str(result["post_url"])
    now = datetime.now(timezone.utc)
    await session.execute(
            text("""
                INSERT INTO seo_agent.social_posts
                ...
                VALUES (:business_id, :job_id, :version_id, :binding_id, :platform, :url,
                        'published', CAST(:snapshot AS jsonb), :now, :now)
            """),
    ...
    await session.execute(
            text("UPDATE seo_agent.social_content_packages SET status = 'published' WHERE id = :id"),
            {"id": row["package_id"]},
        )
...
async def _load_job(session: AsyncSession, job_id: str, *, business_id: str, expected_status: str) -> Any:
    result = await session.execute(
        text("""
            SELECT j.*, p.platform, p.content_type, v.title, v.body, v.metadata,
              ...
              JOIN seo_agent.social_content_packages p ON p.id = j.package_id AND p.business_id = j.business_id
              JOIN seo_agent.social_content_package_versions v ON v.id = j.package_version_id AND v.business_id = j.business_id
              JOIN seo_agent.social_account_bindings b ON b.id = j.binding_id AND b.business_id = j.business_id
              JOIN seo_agent.social_connections c ON c.id = b.connection_id AND c.business_id = j.business_id
...
async def _mark_manual(session: AsyncSession, row: Any, error: str) -> None:
    await session.execute(
        text("""
            UPDATE seo_agent.social_publish_jobs
    ...
    await _insert_attempt(session, row, status="manual_required", evidence={"error": error})
    await session.commit()
...
async def _insert_attempt(session: AsyncSession, row: Any, *, status: str, evidence: dict[str, Any]) -> None:
    await session.execute(
        text("""
            INSERT INTO seo_agent.social_publish_attempts
              (business_id, job_id, attempt_no, status, evidence, finished_at)
            VALUES (:business_id, :job_id,
                    (SELECT COALESCE(MAX(attempt_no), 0) + 1 FROM seo_agent.social_publish_attempts WHERE job_id = :job_id),
...
__all__ = ["confirm_social_job", "prepare_social_job"]
