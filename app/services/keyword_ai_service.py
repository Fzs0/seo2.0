...
async def plan_site_keyword_candidates(
    session: AsyncSession,
    *,
    site_id: str,
    count: int,
    ...
    """Evaluate a small cluster-representative pool for one site's scheduled demand."""
    site = (
        await session.execute(
            text(
                """
                SELECT s.id, s.name, s.site_type, s.business_id, s.strategy_enabled, s.market, s.language_code, s.content_role, s.content_scope,
                       CASE WHEN s.knowledge_profile->>'status' = 'confirmed'
                            THEN s.knowledge_profile ELSE '{}'::jsonb END AS knowledge_profile,
                       (SELECT count(*) FROM seo_agent.posts p WHERE p.site_id = s.id) AS published_posts,
                       (SELECT count(*) FROM seo_agent.tasks t WHERE t.site_id = s.id AND t.status IN ('queued', 'running')) AS active_tasks
                  FROM seo_agent.sites s
                 WHERE s.id = CAST(:site_id AS uuid) AND s.status = 'active' AND s.strategy_enabled = true
                """
            ),
            {"site_id": site_id},
        )
    ...
    if not site:
        raise ValueError("目标站点不存在或未启用")
    if not site.get("business_id"):
        raise ValueError("目标站点缺少业务归属")
    if not site.get("market") or not site.get("language_code"):
        raise ValueError(f"站点 {site['name']} 缺少市场或语种配置")

    candidate_limit = min(300, max(20, count * 10))
    rows = (
        await session.execute(
            text(
                """
                SELECT k.id, k.keyword, k.volume, k.kd, k.cpc, k.intent, k.serp_features,
                       k.trend, k.trend_data, k.pkd, k.potential_traffic, k.competitive_density,
                       k.serp_results, k.keyword_type, k.priority, k.score, k.topic_cluster,
                       k.topic_cluster_id, k.cluster_role, k.cluster_size, k.pillar_keyword,
                       k.market, k.language_code, k.page_group
                  FROM seo_agent.keywords k
                 WHERE k.status IN ('imported', 'analyzed', 'planned')
                   AND COALESCE(k.preflight_status, 'ready') = 'ready'
                   AND COALESCE(k.priority, '') <> 'Hold'
                   AND k.cluster_role IN ('pillar', 'standalone')
                   AND k.volume > 0
                   AND k.assigned_site_id IS NULL
                   AND k.business_id = :business_id
                   AND NOT (COALESCE(k.ai_review, '{}'::jsonb) ?| ARRAY['strategy', 'reservation'])
                   AND lower(k.market) = lower(:market)
                   AND lower(k.language_code) = lower(:language_code)
                   AND NOT EXISTS (
                       SELECT 1 FROM seo_agent.tasks t
                        WHERE t.keyword_id = k.id AND t.status IN ('queued', 'running', 'done')
                   )
                   AND NOT EXISTS (
                       SELECT 1 FROM seo_agent.articles a
                        WHERE a.keyword_id = k.id OR lower(a.primary_keyword) = lower(k.keyword)
                   )
                 ORDER BY k.score DESC NULLS LAST, k.volume DESC NULLS LAST, k.kd ASC NULLS LAST
                 LIMIT :limit
                """
            ),
            {
    ...
    candidates = [
        dict(row)
        for row in rows
        if str(row.get("topic_cluster_id") or row["id"]) not in excluded
    ]
    if not candidates:
        return {"site": dict(site), "items": [], "candidates": 0, "suitable": 0}

    strategies: dict[str, tuple[dict[str, Any], dict[str, Any]]] = {}
    context = {
        "targetSite": dict(site),
        "requiredArticles": count,
        "instruction": "Select only topics genuinely suitable for this site's current role. Do not fill quota with weak matches.",
    }
    for offset in range(0, len(candidates), _AI_REQUEST_SIZE):
        batch = candidates[offset:offset + _AI_REQUEST_SIZE]
        ai = await generate_ai_content(stage="keyword_analysis", prompt=_prompt(batch, None, [dict(site)], context))
        parsed = _parse_json(ai.get("content") or "")
        if not parsed:
            raise ValueError(str(ai.get("error") or ai.get("status") or "AI 未返回有效的关键词规划"))
        for strategy in parsed:
            if not isinstance(strategy, dict):
                continue
            keyword_id = str(strategy.get("keywordId") or strategy.get("id") or "")
            if keyword_id:
                strategies[keyword_id] = (strategy, ai)
        # ponytail: 候选已按质量排序，够本次篇数就停；质量不足时才继续下一批。
        if sum(1 for strategy, _ in strategies.values() if str(strategy.get("relevance") or "needs_review").lower() == "relevant") >= count:
            break

    items: list[dict[str, Any]] = []
    for keyword in candidates:
        pair = strategies.get(str(keyword["id"]))
        if not pair:
            continue
        strategy, ai = pair
        if str(strategy.get("relevance") or "needs_review").lower() != "relevant":
            continue
        confidence = _confidence(strategy.get("confidence"))
        priority = strategy.get("priority") if strategy.get("priority") in {"P0", "P1", "P2", "P3"} else (keyword.get("priority") or "P2")
        strategy = {
            **strategy,
            "assignedSiteId": str(site["id"]),
            "assignedSiteLabel": site["name"],
            "strategy_type": "new_article",
            "query": keyword["keyword"],
            "title": f"新写文章：{keyword['keyword']}",
            "priority": priority,
        ...
        items.append(
            {
                ...
                "topic_cluster": keyword.get("topic_cluster") or keyword["keyword"],
                ...
                "intent": strategy.get("intent") or keyword.get("intent"),
    ...
    items.sort(key=lambda item: (priority_weight.get(item["priority"], 0), item["confidence"], item["score"], item["volume"], -item["kd"]), reverse=True)
    return {"site": dict(site), "items": items[:count], "candidates": len(candidates), "suitable": len(items)}
