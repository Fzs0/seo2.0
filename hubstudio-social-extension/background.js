...
async function requestJson(state, path, options = {}, authorized = false) {
  if (state.backendUrl === "native") {
    return nativeRequest(path, options, authorized ? state.accessToken : null);
  }
...
async function discoverBackend(preferred) {
  ...
  try {
    await nativeRequest("/api/health", { method: "GET" }, null);
    return "native";
  } catch (error) {
    failures.push(`Native Messaging: ${error.message}`);
  }
  throw new Error(`无法连接本机后端（${failures.join("；")}）。请确认本地桥接程序已安装并重新加载扩展。`);
}

async function nativeRequest(path, options = {}, token = null) {
  const response = await chrome.runtime.sendNativeMessage("com.exdivo.publisher", {
    type: "http",
  ...
  if (!response) throw new Error(chrome.runtime.lastError?.message || "本地桥接程序没有响应");
  if (!response.ok) throw new Error(response.error || `本地后端返回 HTTP ${response.status}`);
