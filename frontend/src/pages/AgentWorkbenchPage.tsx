...
export type AgentView = 'command' | 'assets' | 'opportunities' | 'execution' | 'risk' | 'review'
...
function siteLabel(site?: Site) {
  return site ? `${site.name} · ${site.language_code || site.market || '未设置语种'}` : '未分配站点'
}
...
function keywordOpportunity(keyword: Keyword): Opportunity {
  ...
  const score = number(strategy?.confidence ?? keyword.score)
  return {
    id: keyword.id,
    type,
    tone,
  ...
  return (
    <section className="page agent-page" data-screen-label="SEO Agent 工作台">
      ...
      <DataGuard loading={loading} error={error} empty={!sites.data && !keywords.data}>
        {view === 'command' && <CommandView sites={sites.data || []} keywords={keywordItems} articles={articleItems} analytics={analytics.data} opportunities={opportunities} onOpen={setSelectedOpportunity} onNotify={onNotify} />}
        {view === 'assets' && <AssetsView sites={sites.data || []} />}
        {view === 'opportunities' && <OpportunitiesView opportunities={opportunities} onOpen={setSelectedOpportunity} />}
        ...
        {view === 'review' && <ReviewView analytics={analytics.data} articles={articleItems} />}
      </DataGuard>

      {selectedOpportunity && <OpportunityDrawer opportunity={selectedOpportunity} onClose={() => setSelectedOpportunity(null)} onNotify={onNotify} />}
    </section>
  )
}
...
function headerFor(view: AgentView) {
  const headers: Record<AgentView, { eyebrow: string; title: string; description: string }> = {
    ...
    assets: { eyebrow: 'ASSET MAP · 资产地图', title: '资产集群', description: '按市场、语种、站点角色和实际数据组织资产；一个站点可以属于多个视角。' },
    opportunities: { eyebrow: 'OPPORTUNITY ENGINE · 机会引擎', title: '机会队列', description: '机会来自关键词、GSC 查询和站点数据，优先级不再由静态假数据决定。' },
    ...
    review: { eyebrow: 'REVIEW LOOP · 数据复盘', title: '数据复盘', description: '查看 GSC、GA4 和文章资产的真实结果，为下一轮策略提供依据。' },
  }
  return headers[view]
}

function CommandView({ sites, keywords, articles, analytics, opportunities, onOpen, onNotify }: { sites: Site[]; keywords: Keyword[]; articles: Array<{ status: string }>; analytics: ReturnType<typeof useAnalyticsOverview>['data']; opportunities: Opportunity[]; onOpen: (item: Opportunity) => void; onNotify?: (title: string, detail?: string) => void }) {
  ...
  return <>
    ...
    <div className="agent-main-grid">
      ...
      <div className="agent-rail"><AgentLoop keywords={keywords} articles={articles} /><div className="card"><div className="card__title"><span>需要人工判断</span><span className="tag tag--pink">{failedSync} 项</span></div><div className="agent-risk-list">{failedSync ? <div className="agent-risk-item"><span className="agent-risk-dot" /><div><b>数据同步存在失败记录</b><p>请进入同步日志查看具体错误。</p></div></div> : <div className="agent-empty">当前没有后端报告的同步异常。</div>}</div><button type="button" className="btn btn--ghost agent-block-btn" onClick={() => onNotify?.('风险治理视图已准备', '点击顶部“风险治理”查看真实异常。')}>查看风险边界</button></div></div>
    </div>
  </>
}

function AssetsView({ sites }: { sites: Site[] }) {
  return <div className="card"><div className="card__title"><span>资产总览</span><small>{sites.length ? `${sites.length} 个已配置站点` : '暂无站点'}</small></div><div className="agent-asset-grid">{sites.map((site) => <div className="agent-asset-card" key={site.id}><span className={`tag tag--${site.status === 'active' ? 'green' : 'pink'}`}>{site.status}</span><b>{site.name}</b><small>{site.domain} · {site.language_code || '未设置语种'}</small><div><span>{site.content_role || site.site_type}</span><strong>{site.publish_ready ? '可发布' : '未就绪'}</strong></div></div>)}</div>{!sites.length && <div className="agent-empty">后端暂无站点。请先同步 config 目录下的站点配置。</div>}</div>
}

function OpportunitiesView({ opportunities, onOpen }: { opportunities: Opportunity[]; onOpen: (item: Opportunity) => void }) {
  return <div className="card"><div className="card__title"><span>后端机会数据</span><small>{opportunities.length} 条已加载</small></div><OpportunityTable opportunities={opportunities} onOpen={onOpen} /></div>
}
...
function ReviewView({ analytics, articles }: { analytics: ReturnType<typeof useAnalyticsOverview>['data']; articles: Array<{ status: string }> }) {
  const dashboard = analytics?.dashboard
  return <><div className="agent-kpi-row"><MetricCard tone="blue" label="GSC 点击" value={formatNumber(dashboard?.gsc28d?.clicks)} sub="近 28 天" icon="ads_click" /><MetricCard tone="green" label="GSC 展示" value={formatNumber(dashboard?.gsc28d?.impressions)} sub="近 28 天" icon="visibility" /><MetricCard tone="gold" label="GA4 会话" value={formatNumber(dashboard?.ga428d?.sessions)} sub="近 28 天" icon="groups" /><MetricCard tone="pink" label="已保存文章" value={formatNumber(articles.length)} sub="真实文章记录" icon="article" /></div><div className="agent-section-grid"><div className="card"><div className="card__title"><span>GSC 查询机会</span><small>{analytics?.opportunities.length || 0} 条</small></div><div className="agent-queue">{analytics?.opportunities.slice(0, 8).map((row) => <div className="agent-queue-row" key={row.query}><div><b>{row.query}</b><small>点击 {formatNumber(row.clicks)} · 展示 {formatNumber(row.impressions)} · CTR {(row.ctr * 100).toFixed(2)}%</small></div><strong>平均 {number(row.avgPosition).toFixed(1)}</strong></div>)}{!analytics?.opportunities.length && <div className="agent-empty">当前站点暂无 GSC 查询数据。</div>}</div></div><div className="card"><div className="card__title"><span>GA4 渠道</span><small>{analytics?.channels.length || 0} 条</small></div><div className="agent-queue">{analytics?.channels.slice(0, 8).map((channel) => <div className="agent-queue-row" key={channel.channel}><div><b>{channel.channel}</b><small>会话 {formatNumber(channel.sessions)} · 用户 {formatNumber(channel.users)}</small></div><strong>{formatNumber(channel.conversions)} 转化</strong></div>)}{!analytics?.channels.length && <div className="agent-empty">当前站点暂无 GA4 渠道数据。</div>}</div></div></div></>
}

function gscOpportunity(row: GscQuery): Opportunity {
  return { id: `gsc-${row.query}`, type: 'GSC 机会', tone: 'blue', title: row.query, asset: '当前站点', reason: `GSC 已有 ${formatNumber(row.impressions)} 次展示，CTR ${(row.ctr * 100).toFixed(2)}%。`, impact: `${formatNumber(row.clicks)} 点击`, score: row.avgPosition ? `P${row.avgPosition.toFixed(0)}` : '—' }
}
...
function OpportunityTable({ opportunities, onOpen }: { opportunities: Opportunity[]; onOpen: (item: Opportunity) => void }) {
  ...
  return <div className="agent-table-wrap"><table className="agent-table"><thead><tr><th>机会</th><th>资产范围</th><th>AI/数据依据</th><th>影响</th><th>置信度</th><th /></tr></thead><tbody>{opportunities.map((item) => <tr key={item.id} onClick={() => onOpen(item)}><td><span className={`tag tag--${item.tone}`}>{item.type}</span><b>{item.title}</b></td><td>{item.asset}</td><td>{item.reason}</td><td className="agent-strong">{item.impact}</td><td><span className="tag tag--green">{item.score}</span></td><td><button type="button" className="btn btn--ghost btn--xs" onClick={(event) => { event.stopPropagation(); onOpen(item) }}>查看</button></td></tr>)}</tbody></table></div>
}
...
function OpportunityDrawer({ opportunity, onClose, onNotify }: { opportunity: Opportunity; onClose: () => void; onNotify?: (title: string, detail?: string) => void }) {
  return <div className="agent-drawer" role="presentation" onClick={(event) => event.target === event.currentTarget && onClose()}><aside className="agent-drawer-panel"><div className="agent-drawer-head"><div><div className="agent-eyebrow">机会详情 · 后端数据</div><h2>{opportunity.title}</h2></div><button type="button" className="icon-btn" onClick={onClose}>close</button></div><div className="agent-drawer-section"><h3>作用资产</h3><p>{opportunity.asset}</p></div><div className="agent-drawer-section"><h3>数据依据</h3><p>{opportunity.reason}</p></div><div className="agent-drawer-section"><h3>当前状态</h3><p>影响：{opportunity.impact} · 置信度/位置：{opportunity.score}</p></div><div className="agent-drawer-actions"><button type="button" className="btn btn--primary" onClick={() => onNotify?.('已记录人工确认', `${opportunity.title} 已进入后续执行视图。`)}>确认并记录</button><button type="button" className="btn btn--ghost" onClick={onClose}>关闭</button></div></aside></div>
}
