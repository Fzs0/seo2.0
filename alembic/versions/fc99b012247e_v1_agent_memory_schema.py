"""v1 Agent Memory Schema：6 张业务表（sites / keywords / posts / serp_snapshots / tasks / articles）。
...
def upgrade() -> None:
    ...
    op.execute("CREATE SCHEMA IF NOT EXISTS seo_agent;")

    # 共享 trigger function（被 6 张表复用）
    op.execute(
        """
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS sites_market_language_idx ON seo_agent.sites (market, language_code, site_type, status);"
    )
    op.execute(
        """
        DROP TRIGGER IF EXISTS set_sites_updated_at ON seo_agent.sites;
        CREATE TRIGGER set_sites_updated_at BEFORE UPDATE ON seo_agent.sites
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS keywords_status_priority_idx ON seo_agent.keywords (status, priority, score DESC NULLS LAST);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS keywords_assigned_site_idx ON seo_agent.keywords (assigned_site_id, status, priority);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS keywords_text_idx ON seo_agent.keywords (normalized_keyword);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS keywords_raw_gin_idx ON seo_agent.keywords USING gin (raw);"
    )
    op.execute(
        """
        DROP TRIGGER IF EXISTS set_keywords_updated_at ON seo_agent.keywords;
        CREATE TRIGGER set_keywords_updated_at BEFORE UPDATE ON seo_agent.keywords
        FOR EACH ROW EXECUTE FUNCTION seo_agent.set_updated_at();
        """
    )
    ...
    op.execute(
        ...
        ON seo_agent.posts (site_id, external_id) WHERE external_id IS NOT NULL AND external_id <> '';
        """
    )
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS posts_unique_url
        ON seo_agent.posts (site_id, url) WHERE url IS NOT NULL AND url <> '';
        """
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS posts_site_status_idx ON seo_agent.posts (site_id, status, published_at DESC NULLS LAST);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS posts_keyword_idx ON seo_agent.posts (primary_keyword_id, primary_keyword);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS posts_raw_gin_idx ON seo_agent.posts USING gin (raw);"
    )
    op.execute(
        """
        DROP TRIGGER IF EXISTS set_posts_updated_at ON seo_agent.posts;
        CREATE TRIGGER set_posts_updated_at BEFORE UPDATE ON seo_agent.posts
        FOR EACH ROW EXECUTE FUNCTION seo_agent.set_updated_at();
        """
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS serp_keyword_locale_idx ON seo_agent.serp_snapshots (normalized_keyword, google_gl, google_hl, requested_at DESC);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS serp_keyword_id_idx ON seo_agent.serp_snapshots (keyword_id, requested_at DESC);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS serp_organic_gin_idx ON seo_agent.serp_snapshots USING gin (organic_results);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS serp_questions_gin_idx ON seo_agent.serp_snapshots USING gin (related_questions);"
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS tasks_status_priority_idx ON seo_agent.tasks (status, priority, created_at);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS tasks_site_keyword_idx ON seo_agent.tasks (site_id, keyword_id, task_type);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS tasks_payload_gin_idx ON seo_agent.tasks USING gin (payload);"
    )
    op.execute(
        """
        DROP TRIGGER IF EXISTS set_tasks_updated_at ON seo_agent.tasks;
        CREATE TRIGGER set_tasks_updated_at BEFORE UPDATE ON seo_agent.tasks
        FOR EACH ROW EXECUTE FUNCTION seo_agent.set_updated_at();
        """
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS articles_site_status_idx ON seo_agent.articles (site_id, status, created_at DESC);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS articles_keyword_idx ON seo_agent.articles (keyword_id, status);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS articles_parts_gin_idx ON seo_agent.articles USING gin (article_parts);"
    )
    op.execute(
        """
        DROP TRIGGER IF EXISTS set_articles_updated_at ON seo_agent.articles;
        CREATE TRIGGER set_articles_updated_at BEFORE UPDATE ON seo_agent.articles
        FOR EACH ROW EXECUTE FUNCTION seo_agent.set_updated_at();
        """
    )

    # articles.tasks_article_id_fkey（v1 末加上的）
    op.execute(
        """
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS seo_agent.articles CASCADE;")
    op.execute("DROP TABLE IF EXISTS seo_agent.tasks CASCADE;")
    op.execute("DROP TABLE IF EXISTS seo_agent.serp_snapshots CASCADE;")
    op.execute("DROP TABLE IF EXISTS seo_agent.posts CASCADE;")
    op.execute("DROP TABLE IF EXISTS seo_agent.keywords CASCADE;")
    op.execute("DROP TABLE IF EXISTS seo_agent.sites CASCADE;")