...
async def create_package(session: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with session.begin():
        decision_result = await session.execute(
            text("SELECT business_id, status, requested_platforms FROM social.decisions WHERE id = CAST(:id AS uuid) FOR UPDATE"),
        ...
        package_result = await session.execute(
            text("""
                INSERT INTO social.content_packages
...
async def create_package_version(
    ...
    async with session.begin():
        result = await session.execute(
            text("""
                SELECT * FROM social.content_packages
        ...
        await session.execute(
            text("""
                UPDATE social.content_packages
    ...
    result = await session.execute(
        text(f"""
            SELECT p.*, v.id AS version_id, v.language_code, v.title, v.body, v.media,
              ...
              FROM social.content_packages p
              JOIN social.content_package_versions v
...
async def create_binding(session: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    result = await session.execute(
        text("""
            INSERT INTO social.account_bindings
...
async def activate_binding(session: AsyncSession, binding_id: str, *, business_id: str) -> dict[str, Any]:
    ...
    if row["connection_status"] != "verified":
        raise ValueError("binding requires a verified Hubstudio connection")
    ...
    try:
        environment = await adapter.open_environment(str(row["container_code"]))
    except SocialConnectionError as error:
        raise ValueError(str(error)) from error
    update = await session.execute(
        text("""
            UPDATE social.account_bindings
        ...
        """),
        {"id": binding_id, "business_id": business_id,
         "container_name": row["container_name"] or environment.get("container_code") or ""},
    )
    await session.commit()
...
async def list_bindings(session: AsyncSession, *, business_id: str, platform: str | None, limit: int) -> dict[str, Any]:
    ...
    result = await session.execute(
        text(f"SELECT * FROM social.account_bindings WHERE {where} ORDER BY created_at DESC LIMIT :limit"),
        params,
    )
    items = [_row(item) for item in result.mappings().all()]
    return {"items": items, "total": len(items)}
...
async def create_publish_job(
    ...
    async with session.begin():
        replay = await session.execute(
            text("SELECT * FROM social.publish_jobs WHERE business_id = :business_id AND idempotency_key = :key"),
        ...
        source_result = await session.execute(
            text("""
                SELECT p.platform, p.status AS package_status, p.current_version,
                  ...
                  JOIN social.account_bindings b
        ...
        job_result = await session.execute(
            text("""
                INSERT INTO social.publish_jobs
        ...
        await session.execute(
            text("UPDATE social.content_packages SET status = 'queued' WHERE id = CAST(:id AS uuid) AND business_id = :business_id"),
            {"id": package_id, "business_id": business_id},
        )
...
async def list_publish_jobs(session: AsyncSession, *, business_id: str, status: str | None, limit: int) -> dict[str, Any]:
    ...
    result = await session.execute(
        text(f"""
            SELECT j.*, p.platform, p.content_type, b.display_name, b.container_code,
              ...
              JOIN social.content_packages p ON p.id = j.package_id
    ...
    items = [
        {key: value for key, value in _row(item).items() if key != "executor_confirmation"}
...
async def cancel_publish_job(session: AsyncSession, job_id: str, *, business_id: str) -> dict[str, Any]:
    async with session.begin():
        result = await session.execute(
            text("""
                UPDATE social.publish_jobs
                   SET status = 'cancelled', executor_confirmation = NULL
                 WHERE id = CAST(:id AS uuid) AND business_id = :business_id
                   AND status IN ('pending', 'awaiting_review', 'manual_required', 'failed')
             RETURNING *
            """),
        ...
        row = result.mappings().first()
        if not row:
            raise ValueError("only pending, awaiting_review, failed, or manual_required jobs can be cancelled")
        await session.execute(
            text("UPDATE social.content_packages SET status = 'cancelled' WHERE id = :id"),
            {"id": row["package_id"]},
        )
    return _row(row)
...
async def _insert_version(
    ...
    result = await session.execute(
        text("""
            INSERT INTO social.content_package_versions
...
async def _change_package_status(
    ...
    result = await session.execute(
        text("""
            UPDATE social.content_packages SET status = :target
             WHERE id = CAST(:id AS uuid) AND business_id = :business_id
