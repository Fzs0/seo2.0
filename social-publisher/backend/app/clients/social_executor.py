...
class SocialExecutorClient:
    ...
    async def command(self, payload: dict[str, Any]) -> dict[str, Any]:
        client = self._client or httpx.AsyncClient(
            # A platform verifier may legitimately run after Puppeteer's
            # per-operation timeout. Keep this above the executor's bounded
            # browser work so the HTTP client never abandons a live click.
            # A batch must not abandon a live browser operation and then start
            # the next job against the same Hubstudio container. Video
            # processing/verification can legitimately take several minutes.
            timeout=900, follow_redirects=False, trust_env=False,
        )
